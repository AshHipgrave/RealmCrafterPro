...
std::wstring WCPattern::classNegate     (std::wstring s1)                  const
{
  ...
  for (i = 0; i < (int)s1.size(); ++i)
  {
    ...
    {
      if      (s1[i] >= (wchar_t)'a' && s1[i] <= (wchar_t)'z') m[s1[i] - (wchar_t)'a' + (wchar_t)'A'] = 1;
      else if (s1[i] >= (wchar_t)'A' && s1[i] <= (wchar_t)'Z') m[s1[i] - (wchar_t)'A' + (wchar_t)'a'] = 1;
    }
  }
...
std::wstring WCPattern::parsePosix()
{
