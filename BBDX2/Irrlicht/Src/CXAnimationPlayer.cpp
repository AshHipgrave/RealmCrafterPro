...
CXAnimationPlayer::~CXAnimationPlayer()
{
	...
	for(int i = 0; i < this->AnimatedMesh->getMeshBufferCount(); ++i)
	{
		IMeshBuffer* MeshBuffers = this->AnimatedMesh->getMeshBuffer(i);

		if(MeshBuffers->getMaterial().Texture1)
		{
			if(MeshBuffers->getMaterial().Texture1->ReferenceCounter == 1)
				Driver->removeTexture(MeshBuffers->getMaterial().Texture1);
			else
				MeshBuffers->getMaterial().Texture1->drop();
			if(MeshBuffers->getMaterial().Texture1->ReferenceCounter == 1)
				Driver->removeTexture(MeshBuffers->getMaterial().Texture1);
		}

		if(MeshBuffers->getMaterial().Texture2)
		{	
			if(MeshBuffers->getMaterial().Texture2->ReferenceCounter == 1)
				Driver->removeTexture(MeshBuffers->getMaterial().Texture2);
			else
				MeshBuffers->getMaterial().Texture2->drop();
			if(MeshBuffers->getMaterial().Texture2->ReferenceCounter == 1)
				Driver->removeTexture(MeshBuffers->getMaterial().Texture2);
		}

		if(MeshBuffers->getMaterial().Texture3)
		{
			if(MeshBuffers->getMaterial().Texture3->ReferenceCounter == 1)
				Driver->removeTexture(MeshBuffers->getMaterial().Texture3);
			else
				MeshBuffers->getMaterial().Texture3->drop();
			if(MeshBuffers->getMaterial().Texture3->ReferenceCounter == 1)
				Driver->removeTexture(MeshBuffers->getMaterial().Texture3);
		}

		if(MeshBuffers->getMaterial().Texture4)
		{
			if(MeshBuffers->getMaterial().Texture4->ReferenceCounter == 1)
				Driver->removeTexture(MeshBuffers->getMaterial().Texture4);
			else
				MeshBuffers->getMaterial().Texture4->drop();
			if(MeshBuffers->getMaterial().Texture4->ReferenceCounter == 1)
				Driver->removeTexture(MeshBuffers->getMaterial().Texture4);
		}
	}
