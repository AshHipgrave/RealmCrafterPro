...
/*inline IDirect3DDevice9* CNullDriver::GetDXDevice()
{
	return 0;
}*/

irr::core::stringc GetFileNameUpper(irr::core::stringc ln)
{
	irr::core::stringc fi = "none";
	for(irr::s32 i = ln.size(); i > 0; --i)
	{
		if(ln.subString(i,1) == "\\")
		{
			fi = ln.subString(i + 1, ln.size() - i - 1);
			break;
		}
	}
	fi.make_upper();
...
void CNullDriver::setTexture(s32 stage, video::ITexture* texture)
{
...
void CNullDriver::setDefaultTexture(ITexture* tex)
{
	deftex = tex;
}

ITexture* CNullDriver::getDefaultTexture()
{
	return deftex;
}
...
//! loads a Texture
ITexture* CNullDriver::getTexture(const c8* filename, bool Mask, bool Cubemap, bool Volume)
{
	...
	texture = loadTextureFromFile(filename, Mask, Cubemap, Volume);

	if (texture)
	{
		addTexture(texture);
		// No longer drop.
		// RefCount will be 2 at this point due to cache and
		// one 'alive' texture.
...
//! loads a Texture
// ITexture* CNullDriver::getTexture(io::IReadFile* file, bool Mask, bool Cubemap, bool Volume)
// {
// 	ITexture* texture = 0;
// 
// 	if (file)
...
// 			return texture;
// 
// 		texture = loadTextureFromFile(file, Mask, Cubemap, Volume);
// 
// 		if (texture)
// 		{
// 			addTexture(texture);
...
void CNullDriver::maskTexture(ITexture* tex)
{
		//SColor* dat = (SColor*)tex->lock();

		//for(int yy = 0; yy < tex->getSize().Height; ++yy)
		//{
		//	for(int xx = 0; xx < tex->getSize().Width; ++xx)
		//	{
		//		if(dat[(yy*tex->getSize().Width)+xx].getRed() == 0 && dat[(yy*tex->getSize().Width)+xx].getGreen() == 0 && dat[(yy*tex->getSize().Width)+xx].getBlue() == 0 )
		//		{
		//			dat[(yy*tex->getSize().Width)+xx].setAlpha(0);
		//		}
		//	}
		//}
...
ITexture* CNullDriver::getEmptyTexture(irr::c8* filename)
{
	OutputDebugString("Don't Call Me!\n");
	return 0;
}

//! opens the file and loads it into the surface
video::ITexture* CNullDriver::loadTextureFromFile(const irr::c8* file, bool Mask, bool Cubemap, bool Volume)
{
	ITexture* texture = 0;
	bool Alpha = false;

	core::stringc fnu = GetFileNameUpper(file);
	if(fnu.subString(0,2) == "M_")
		Mask = true;
	if(fnu.subString(0,2) == "A_")
		Alpha = true;

	texture = loadDDSTexture((irr::c8*)file, Mask, Cubemap, Volume);
...
ITexture* CNullDriver::loadDDSTexture(irr::c8 *filename, bool Mask, bool Cubemap, bool Volume)
{
	return 0;
}
...
video::ITexture* CNullDriver::findTexture(const c8* filename)
{
	...
	for(u32 i = 0; i < Textures.size(); ++i)
	{
		core::stringc tname = Textures[i].Surface->getName();
		tname.make_lower();
		if(file == tname)
			return Textures[i].Surface;	
	}
...
{
	return 0;//new CSoftwareTexture(surface, name);
}
...
void CNullDriver::drawIndexedTriangleList(const S3DVertex* vertices, s32 vertexCount, const u32* indexList, s32 triangleCount)
{
...
//! draws an indexed triangle list
void CNullDriver::drawIndexedTriangleList(const S3DVertex2TCoords* vertices, s32 vertexCount, const u32* indexList, s32 triangleCount)
{
	PrimitivesDrawn += triangleCount;
}
...
void CNullDriver::drawIndexedTriangleList(const S3DVertexTangents* vertices,
	s32 vertexCount, const u32* indexList, s32 triangleCount)
{
	PrimitivesDrawn += triangleCount;
}
							...
							const core::vector3df& end, SColor color)
{
	...
	u32 idx[12] = {0,1,2, 0,2,1,   0,1,3, 0,3,1};
...
void CNullDriver::drawMeshBuffer(scene::IMeshBuffer* mb)
{
	this->drawMeshBuffer(mb, false);
}

void CNullDriver::drawMeshBuffer(scene::IMeshBuffer* mb, bool IsCustom)
{
	if (!mb)
		return;

	if ((mb->indexB!=0)&&(mb->vertexB!=0))
		this->drawIndexedTriangleListHw(
			mb->indexB,
			mb->vertexB,
			mb->getIndexCount(),
			mb->getVertexCount(),
			IsCustom ? video::EVT_ANIMATEDVERT : mb->getVertexType(),
			mb->getIndexCount() / 3
			);
...
void CNullDriver::drawIndexedTriangleListHw(void* indexP,void* vertexP,int indexC,int vertexC,video::E_VERTEX_TYPE vt, int TCount)
{
	return;
}


bool CNullDriver::updateHardwareBuffer(scene::IMeshBuffer* buffer)
{
	return false;
}

bool CNullDriver::deleteHardwareBuffer(scene::IMeshBuffer* buffer)
{
	return false;
}
