...
namespace irr
{
	namespace video
	{
		...
		IVideoDriver* createDirectX9Driver(const core::dimension2d<s32>& screenSize, HWND window, 
			u32 bits, bool fullscreen, bool stencilbuffer, io::IFileSystem* io,
			bool pureSoftware, bool highPrecisionFPU, bool vsync, int antiAlias);
	...
	FullScreen(fullscreen), ExternalWindow(false)
{
	...
	if (driverType != video::EDT_NULL && externalWindow==0)
	{
		...
		wcex.hIcon = (HICON)LoadImage(hInstance, "wnd.ico", IMAGE_ICON, 0,0, LR_LOADFROMFILE); 
		...
		if (!fullscreen)
			style = WS_OVERLAPPED | WS_CAPTION;//WS_SYSMENU | WS_BORDER | WS_CAPTION | WS_CLIPCHILDREN | WS_CLIPSIBLINGS;
				   ...
				   bool highPrecisionFPU)
{
	...
	default:
		MessageBox(NULL, "Error: No device selected", "BBDX Error", MB_OK);
		exit(0);
		break;
	}
}
