...
bool CAnimatedMeshB3d::ReadChunkTEXS(io::IReadFile* file, B3dChunk *B3dStack, s16 &B3dStackSize)
{
	...
	while(B3dStack[B3dStackSize].startposition + B3dStack[B3dStackSize].length>file->getPos()) //this chunk repeats
	{
		...
		B3dTexture.Texture=Driver->getTexture ( TextureName.c_str(), Masking );
	...
	while(B3dStack[B3dStackSize].startposition + B3dStack[B3dStackSize].length>file->getPos()) //this chunk repeats
	{
		...
		for(s32 i=0;i<3;i++)
		{
			if (AnimatedVertices_VertexID[ vertex_id[i] ]==-1)
			{
				...
				irr::video::S3DVertex2TCoords *Vertex = Vertices[vertex_id[i]];
...
//char fg[128];
//sprintf(fg,"TEXS: %f, %f",B3dMaterial->Textures[0]->Xscale, B3dMaterial->Textures[0]->Yscale);
//MessageBox(NULL, fg, "", MB_OK);
...
bool CAnimatedMeshB3d::loadFile(io::IReadFile* file)
{
	...
	//core::stringc Test = "Data\\Meshes\\Weapons & Staves\\maces\\hammer_02.eb3d";
	//FName.make_lower();
	//Test.make_lower();
	//if(FName == Test)
	//{
	//	FILE* F = fopen("tmp.b3d", "wb");
	...
	{
		SB3DMeshBuffer* MeshBuffer = AnimatedVertices_MeshBuffer[ i ];
		if(MeshBuffer)
		{
			...
			if(VertexID >= 0)
			{
				...
				{
					video::S3DVertex2TCoords* Vert = &MeshBuffer->Vertices[VertexID];
					VertexBlends VBL;
					VBL.Vert = Vert;
					int BoneCnt = 0;
	...
	else
	{
		...
		for(int i = 0; i < this->getMeshBufferCount(); ++i)
		{
			...
			for(int f = 0; f < O->getVertexCount(); ++f)
				M->Vertices.push_back(video::S3DVertexTangents(O->Vertices[f].Pos, O->Vertices[f].TCoords, O->Vertices[f].Color, O->Vertices[f].Normal));
