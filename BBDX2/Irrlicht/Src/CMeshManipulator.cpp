...
inline void makePlanarMappingT(VERTEXTYPE *v,
				   int vtxcnt,
				   u32* idx, int idxcnt, f32 resolution)
{
	for (int i=0; i<idxcnt; i+=3)
	{
		core::plane3d<f32> p(v[idx[i+0]].Pos, v[idx[i+1]].Pos, v[idx[i+2]].Pos);
...
SMesh* CMeshManipulator::createMeshCopy(scene::IMesh* mesh) const
{
	...
	for (s32 b=0; b<meshBufferCount; ++b)
	{
		...
		const u32* idx = mesh->getMeshBuffer(b)->getIndices();
...
IMesh* CMeshManipulator::createMeshWithTangents(IMesh* clone) const
{
	//if (!mesh)
	//	return 0;
	...
	s32 meshBufferCount = clone->getMeshBufferCount();
	s32 b;
	...
	for (b=0; b<meshBufferCount; ++b)
	{
		...
		u32* idx = clone->getMeshBuffer(b)->getIndices();
		video::S3DVertexTangents* v =
