...
namespace video
{
	...
	public:
		...
		//! loads a Texture
		virtual ITexture* getTexture(const c8* filename, bool Mask = false, bool Cubemap = false, bool Volume = false);
		//virtual ITexture* getTexture(const c8* filename);

		//! loads a Texture
		//virtual ITexture* getTexture(io::IReadFile* file, bool Mask = false, bool Cubemap = false, bool Volume = false);
		//virtual ITexture* getTexture(io::IReadFile* file);

		virtual ITexture* getEmptyTexture(irr::c8* filename);

		//! Masks a texture
		virtual void maskTexture(ITexture* tex);
		...
		virtual void drawIndexedTriangleList(const S3DVertex* vertices, s32 vertexCount, const u32* indexList, s32 triangleCount);

		//! draws an indexed triangle list
		virtual void drawIndexedTriangleList(const S3DVertex2TCoords* vertices, s32 vertexCount, const u32* indexList, s32 triangleCount);
		...
		virtual void drawMeshBuffer(scene::IMeshBuffer* mb, bool IsCustom);
		...
		virtual void drawIndexedTriangleListHw(void* indexP,void* vertexP,int indexC,int vertexC,video::E_VERTEX_TYPE vt, int tCnt);

		virtual bool updateHardwareBuffer(scene::IMeshBuffer* buffer);
		virtual bool deleteHardwareBuffer(scene::IMeshBuffer* buffer);
		virtual bool updateAnimatedHardwareBuffer(scene::IMeshBuffer* buffer, core::array<scene::VertexBlends>* Blends) { return false;}


		virtual void setDefaultTexture(ITexture* tex);
		virtual ITexture* getDefaultTexture();
		virtual ITexture* loadDDSTexture(c8* filename, bool Mask, bool Cubemap, bool Volume);

		virtual u32 loadFXShader(const c8* linky) { return 0; } 
		virtual void PostProcess() { }
		virtual u32 CreatePPE(u32 Shader) { return 0; };
		virtual void SetPPEActive(u32 PPE, bool Active) { };

		virtual IDirect3DVertexDeclaration9*  GetStandardDefinition(){return 0;}
		virtual void ResetVertexDeclarationCache(){}
		virtual void ResizeScreen(irr::core::dimension2d<irr::s32> NewSize, bool FullScreen, int AntiAlias, int Anisotropic){}
		virtual void* LoadFont(const char* FontName, int Size, int Bold, int Italic){return 0;}
		virtual void FreeFont(void* Font){}

		virtual void setTexture(s32 stage, video::ITexture* texture);
		virtual void Destroy();

		virtual void setShadowMapSize(int newSize) {};
		virtual int getShadowMapSize() { return 0; };
	...
	protected:
		...
		video::ITexture* loadTextureFromFile(const c8* file, bool Mask = false, bool Cubemap = false, bool Volume = false);
