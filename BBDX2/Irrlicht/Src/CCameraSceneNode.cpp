...
void CCameraSceneNode::setViewMatrix(const core::matrix4& newView)
{
	View = newView;
	D3DXMATRIX M = *(D3DXMATRIX*)&View;
	Target   = *(vector3df*)&(Mat4GetPosition(M) + Mat4GetForward(M));
	UpVector = *(vector3df*)&(Mat4GetUp(M));
}
...
void CCameraSceneNode::OnPreRender()
{
	...
	if (SceneManager->getActiveCamera() == this)
	{
		...
		((irr::video::CD3D9Driver*)driver)->setTransformN(video::ETS_PROJECTION, Projection);
		...
		View = getAbsoluteTransformation();
		View.makeInverse();
		recalculateViewArea();
...
{	
	...
	((irr::video::CD3D9Driver*)driver)->setTransformN(video::ETS_VIEW, View);
}
...
void CCameraSceneNode::recalculateViewArea()
{
	...
	//core::matrix4 tmat;
	//tmat.setScale(vector3df(0.2f,0.2f,0.2f));
	//ViewArea.transform(tmat);
}
