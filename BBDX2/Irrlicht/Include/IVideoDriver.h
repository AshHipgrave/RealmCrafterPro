...
namespace video
{
	...
	*/
	...
	public:
		...
		virtual ITexture* getTexture(const c8* filename, bool Mask, bool Cubemap, bool Volumetexture) = 0;

		virtual ITexture* getEmptyTexture(irr::c8* filename) = 0;
		...
		//virtual ITexture* getTexture(io::IReadFile* file, bool Mask, bool Cubemap, bool Volumetexture) = 0;
		
		virtual void maskTexture(ITexture* tex) = 0;
		...
		*/
		...
		virtual void drawIndexedTriangleList(const S3DVertex* vertices,
			s32 vertexCount, const u32* indexList, s32 triangleCount) = 0;
		...
		virtual void drawMeshBuffer(scene::IMeshBuffer* mb, bool IsCustom) = 0;
		...
		virtual bool updateHardwareBuffer(scene::IMeshBuffer* buffer) = 0;
		virtual bool updateAnimatedHardwareBuffer(scene::IMeshBuffer* buffer, core::array<scene::VertexBlends>* Blends) = 0;
		//virtual bool updateHardwareBufferLM(scene::SMeshBufferLightMap* buffer) = 0;
		virtual bool deleteHardwareBuffer(scene::IMeshBuffer* buffer) = 0;

		virtual void setDefaultTexture(ITexture* tex) = 0;
		virtual ITexture* getDefaultTexture() = 0;

		virtual u32 loadFXShader(const c8* linky) = 0;
		virtual void drawIndexedTriangleListHw(void* indexP,void* vertexP,int indexC,int vertexC,video::E_VERTEX_TYPE vt,int TCount) = 0;
		virtual void PostProcess() = 0;	
		virtual u32 CreatePPE(u32 Shader) = 0;
		virtual void SetPPEActive(u32 PPE, bool Active) = 0;
		virtual IDirect3DVertexDeclaration9* GetStandardDefinition() = 0;
		virtual void ResetVertexDeclarationCache() = 0;
		virtual void ResizeScreen(irr::core::dimension2d<irr::s32> NewSize, bool FullScreen, int Antialias, int Anisotropy) = 0;
		virtual void* LoadFont(const char* FontName, int Size, int Bold, int Italic) = 0;
		virtual void FreeFont(void* Font) = 0;

		virtual void setTexture(s32 stage, video::ITexture* texture) = 0;

		virtual void Destroy() = 0;

		virtual void setShadowMapSize(int newSize) = 0;
