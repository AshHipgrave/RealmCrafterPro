	...
	public:
		...
		ISceneNode(	ISceneNode* parent, ISceneManager* mgr, s32 id=-1,
			...
			: RelativeTranslation(position), RelativeRotation(rotation), RelativeScale(scale),
				...
				DebugDataVisible(false), IsDebugObject(false), RenderMask(0),
				lastLOD(0)
		{
			if (Parent)
		...
		{
/*irr::core::stringc N = Tag;
irr::core::stringc Emi = "Emitter";
irr::core::stringc E = N.subString(0, 7);
if(N.size() > 5 && E == Emi)
{
	printf("%x: %s(%s, %i)\n", this, this->getTag(), IsVisible ? "Visible" : "Hidden", this->Effect);
}*/
			if (IsVisible)
			{
		...
		virtual void WriteDesc(irr::io::IWriteFile* File, core::stringc Tabbage)
		{
			Tabbage += "    ";
			core::list<ISceneNode*>::Iterator it = Children.begin();
				for (; it != Children.end(); ++it)
					if(*it != 0)
						(*it)->WriteSceneGraph(File, Tabbage);
		}
		...
		{
			//if(newParent == 0)
			//	newParent = (ISceneNode*)SceneManager;
		...
		// Get/Set the FX file
		virtual void setEffect(u32 E)
		{
			Effect = E;
		}

		virtual u32 getEffect()
		{
			if(Profile == 0)
				return Effect;
			
			return Profile->GetEffect(this, SceneManager);
		}
		...
		virtual void SetParameterValue(irr::core::stringc Name, EParameterType Type, void* Data)
		{
			for(int i = 0; i < EffectParameters.size(); ++i)
			{
				if(EffectParameters[i].Name == Name)
				{
					EffectParameters[i].Type = Type;
					delete EffectParameters[i].Value;
					EffectParameters[i].Value = Data;
					return;
				}
			}
			...
			P.Type = Type;
			P.Value = Data;
			EffectParameters.push_back(P);

		}


		virtual void setShadowLevel(s32 shadowLevel)
		{
			ShadowLevel = shadowLevel;
		}

		virtual s32 getShadowLevel()
		{
			return ShadowLevel;
		}

		virtual void setShadowShader(u32 shadowShader)
		{
			ShadowShader = shadowShader;
		}

		virtual u32 getShadowShader()
		{
			return ShadowShader;
		}

		s32 GetLastLOD() const { return lastLOD; }

		// Public collision variables
		s32 EntityType;
		bool Pickable;
		int NxHandleType;
		void* NxHandle;
		irr::core::array<void*> TriMeshArray;
		irr::core::array<irr::scene::CollisionHitInstance> CollisionHits;
		...
		void* FXHandle;
		bool TriangleCulling;

		//JB: Used for effect constants
		irr::core::array<SEffectParameter> EffectParameters;
		core::array<int> RenderTimes;

		virtual SShaderProfile* GetProfile() { return Profile; }
		virtual void SetProfile(SShaderProfile* Profile) { this->Profile = Profile; }
	...
	protected:
		...
		u32 Effect;
		u32 ShadowShader;
		SShaderProfile* Profile;
		s32 ShadowLevel;

		//! Last LOD (for CAnimatedMeshSceneNode)
		s32 lastLOD;
