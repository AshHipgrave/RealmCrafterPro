...
{
    public class GMoveTool : TransformTool
    {
        ...
        public EventHandler ObjectMoving;
        public EventHandler ObjectMoved;

        public GMoveTool(Entity parent)
        {
            BuildMesh();
            Parent = parent;
        }

        public GMoveTool(Entity parent, EventHandler objectMoving, EventHandler objectMoved)
        {
            Parent = parent;
        ...
        public override void Update(int x, int y)
        {
            ...
            Vector3 Pos = null;
            if(Parent != null)
                Pos = new Vector3(Parent.X(true), Parent.Y(true), Parent.Z(true));
            else
                Pos = new Vector3();

            if (SelectedX || SelectedZ)
            {
                ...
                {
                    if (SelectedX)
                        Pos.X = RenderWrapper.bbdx2_ProjectedX() - Offset.X;
                    if (SelectedZ)
                        Pos.Z = RenderWrapper.bbdx2_ProjectedZ() - Offset.Z;
                }
            }
            ...
            if (SelectedY)
                if (!SelectedX && RenderWrapper.bbdx2_RayToPlane(1, 0, 0, Parent.X(true), Parent.Y(true), Parent.Z(true), LineStart.X, LineStart.Y, LineStart.Z, LineEnd.X, LineEnd.Y, LineEnd.Z) > 0)
                    Pos.Y = RenderWrapper.bbdx2_ProjectedY() - Offset.Y;
...
//                     Pos.Z = RenderWrapper.bbdx2_ProjectedZ() - Offset.Z;
            ...
            if (Parent != null)
            {
                //Entity.TFormVector(Pos.X, Pos.Y, Pos.Z, Parent, null);
                ...
                float[] UnTransformed = new float[] { Parent.X(true) - Pos.X, Parent.Y(true) - Pos.Y, Parent.Z(true) - Pos.Z };

                GCHandle RotHand = GCHandle.Alloc(Rot, GCHandleType.Pinned);
                GCHandle TransformedHand = GCHandle.Alloc(Transformed, GCHandleType.Pinned);
                IntPtr pRot = RotHand.AddrOfPinnedObject();
                IntPtr pTransformed = TransformedHand.AddrOfPinnedObject();

                RenderWrapper.D3DXMatrixRotationYawPitchRoll(pRot,
                    Parent.Yaw(true) * ((float)(Math.PI / 180.0f)),
                    Parent.Pitch(true) * ((float)(Math.PI / 180.0f)),
                    Parent.Roll(true) * ((float)(Math.PI / 180.0f)));
                RenderWrapper.D3DXMatrixInverse(pRot, null, pRot);

                RenderWrapper.D3DXVec3Transform(pTransformed, UnTransformed, pRot);

                RotHand.Free();
                TransformedHand.Free();

                //Parent.Translate(Pos.X, Pos.Y, Pos.Z);

                Parent.Translate(-Transformed.X, -Transformed.Y, -Transformed.Z);

                //Parent.Translate(Pos.X - Parent.X(true), Pos.Y - Parent.Y(true), Pos.Z - Parent.Z(true));
                //Parent.Translate(0, 0.03f, 0.0f, true);
                //Parent.Translate(Parent.X(true) - Pos.X, Parent.Y(true) - Pos.Y, Parent.Z(true) - Pos.Z);
                //Parent.Position(Pos.X, Pos.Y, Pos.Z, true);
            }

            if (SelectedX || SelectedY || SelectedZ)
                if (ObjectMoving != null)
                    ObjectMoving(this, EventArgs.Empty);
