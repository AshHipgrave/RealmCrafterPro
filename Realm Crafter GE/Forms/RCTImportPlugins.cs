...
namespace RCTTest
{
    public static class RCTImportPlugins
    {
        public static List<IRCTPlugin> Plugins = new List<IRCTPlugin>();
            ...
            try
            {
                ...
                foreach (String PluginPath in Files)
                {
                    if (Path.GetExtension(PluginPath).Equals(".dll", StringComparison.CurrentCultureIgnoreCase)
                        && !Path.GetFileNameWithoutExtension(PluginPath).Equals("RCTPlugin.dll", StringComparison.CurrentCultureIgnoreCase))
                    {
        ...
        private static void LoadPlugin(String path)
        {
            try
            {
                ...
                foreach (Type type in LoadingAssembly.GetTypes())
                {
                    ...
                    if (type.IsDefined(typeof(RCTPluginAttribute), true))
                ...
                foreach (Type PluginType in PluginTypes)
                {
                    if (PluginType != null)
                    {
                        IRCTPlugin Provider = Activator.CreateInstance(PluginType) as IRCTPlugin;
                        if (Provider != null)
