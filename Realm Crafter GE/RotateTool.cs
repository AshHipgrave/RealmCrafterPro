...
namespace RealmCrafter_GE
{
    public class RotateTool : TransformTool
    {
        ...
        Vector3 MeshRotation = new Vector3(1, 1, 1);

        public EventHandler ObjectRotating;
        public EventHandler ObjectRotated;

        public RotateTool(Entity parent)
        {
            BuildMesh();
            Parent = parent;
        }

        public RotateTool(Entity parent, EventHandler objectRotating, EventHandler objectRotated)
        {
            Parent = parent;
            if (Parent != null)
                MeshRotation = new Vector3(Parent.Pitch(), Parent.Yaw(), Parent.Roll());
            ObjectRotating = objectRotating;
        ...
        public override void Update(int x, int y)
        {
            ...
            if (SelectedX)
                if (RenderWrapper.bbdx2_RayToPlane(0, 1, 0, Parent.X(), Parent.Y(), Parent.Z(), LineStart.X, LineStart.Y, LineStart.Z, LineEnd.X, LineEnd.Y, LineEnd.Z) > 0)
                    Rotation.X = RenderWrapper.bbdx2_ProjectedZ() - Offset.Z;

            if (SelectedY)
                if (!SelectedX && RenderWrapper.bbdx2_RayToPlane(1, 0, 0, Parent.X(), Parent.Y(), Parent.Z(), LineStart.X, LineStart.Y, LineStart.Z, LineEnd.X, LineEnd.Y, LineEnd.Z) > 0)
                    Rotation.Y = RenderWrapper.bbdx2_ProjectedY() - Offset.Y;
                else if (!SelectedZ && RenderWrapper.bbdx2_RayToPlane(0, 0, 1, Parent.X(), Parent.Y(), Parent.Z(), LineStart.X, LineStart.Y, LineStart.Z, LineEnd.X, LineEnd.Y, LineEnd.Z) > 0)
                    Rotation.Y = RenderWrapper.bbdx2_ProjectedY() - Offset.Y;

            if (SelectedZ)
                if (RenderWrapper.bbdx2_RayToPlane(0, 1, 0, Parent.X(), Parent.Y(), Parent.Z(), LineStart.X, LineStart.Y, LineStart.Z, LineEnd.X, LineEnd.Y, LineEnd.Z) > 0)
                    Rotation.Z = -(RenderWrapper.bbdx2_ProjectedX() - Offset.X);

            if (Parent != null)
                Parent.Rotate(
                    (Rotation.X * 4.0f) + MeshRotation.X,
                    (Rotation.Y * 8.0f) + MeshRotation.Y,
                    (Rotation.Z * 4.0f) + MeshRotation.Z);

            if (SelectedX || SelectedY || SelectedZ)
                if (ObjectRotating != null)
                    ObjectRotating(this, EventArgs.Empty);

            if (Parent != null)
                Mesh.Position(Parent.X(), Parent.Y(), Parent.Z());
        }
        ...
        public override bool MouseDown(Entity camera, int x, int y)
        {
            ...
            if (Rotation != null)
                this.MeshRotation = new Vector3(Parent.Pitch(), Parent.Yaw(), Parent.Roll());
            else
                this.MeshRotation = new Vector3(0, 0, 0);
            ...
            if (Parent != null)
                Center = new Vector3(Parent.X(), Parent.Y(), Parent.Z());

            Vector3 Camera = new Vector3(0, 0, 0);
            if (camera != null)
                Camera = new Vector3(camera.X(), camera.Y(), camera.Z());
        ...
        public override void MouseUp()
        {
            if (SelectedX || SelectedY || SelectedZ)
                if (ObjectRotated != null)
                    ObjectRotated(this, EventArgs.Empty);
