...
#include "SD3D9TreeLODVertex.h"

namespace LT
{
	CD3D9TreeLODMeshBuffer::CD3D9TreeLODMeshBuffer(IDirect3DDevice9* device)
	...
	CD3D9TreeLODMeshBuffer::~CD3D9TreeLODMeshBuffer()
	{
		if(VertexBuffer != 0)
	...
	bool CD3D9TreeLODMeshBuffer::Set() const
	{
		if(FAILED(Device->SetIndices(IndexBuffer)))
			return false;

		if(FAILED(Device->SetStreamSource(0, VertexBuffer, 0, sizeof(SD3D9TreeLODVertex))))
			return false;

		return true;
	}

	bool CD3D9TreeLODMeshBuffer::Draw(unsigned int &trianglesDrawn) const
	{
	...
	CD3D9TreeLODMeshBuffer* CD3D9TreeLODMeshBuffer::CreateD3D9TreeLODMeshBuffer(
		...
		const unsigned short* indices, unsigned int indexCount)
	{
		...
		HRESULT Hr = device->CreateVertexBuffer(
			sizeof(SD3D9TreeLODVertex) * vertexCount, D3DUSAGE_WRITEONLY,
		...
		SD3D9TreeLODVertex* LockedVertices = 0;
		...
		for(unsigned int i = 0; i < vertexCount; ++i)
		{
			...
			LockedVertices[i].SetBillboardScale(vertices[i].BillScaleX, vertices[i].BillScaleY);
		}
		...
		// Create
		CD3D9TreeLODMeshBuffer* Buffer = new CD3D9TreeLODMeshBuffer(device);
	...
	unsigned int CD3D9TreeLODMeshBuffer::GetTriangleCount() const
	{
		return IndexCount / 3;
	}
}
