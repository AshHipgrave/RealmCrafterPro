...
namespace RealmCrafter
{
	...
	NGin::Type CControlEditor2::TypeOf()
	{
		return NGin::Type("RCCE", "RealmCrafter Controls Editor");
	}

	// Create an instance of the control
	CControlEditor2* CControlEditor2::Create(NGin::GUI::IGUIManager* manager, std::string name, NGin::Math::Vector2 &location, NGin::Math::Vector2 &size)
	{
		CControlEditor2* CEditor = new CControlEditor2(Vector2(1, 1), manager);
		CEditor->Locked = true;
		CEditor->Name = name;
	...
	// Constructor
	CControlEditor2::CControlEditor2(NGin::Math::Vector2& ScreenScale, IGUIManager* Manager)
	...
	CControlEditor2::~CControlEditor2()
	{
	}

	void CControlEditor2::OnDeviceReset()
	{
		IControl::OnDeviceReset();

		CControlEditor2::OnTransform();
	}

	bool CControlEditor2::Update(GUIUpdateParameters* Parameters)
	{
		if(!_Enabled)
			return true;
	...
	void CControlEditor2::Initialize()
	{
		...
		Scrollbar->Scroll()->AddEvent(this, &CControlEditor2::Scrollbar_Scroll);
	}

	void CControlEditor2::Scrollbar_Scroll(IControl* sender, EventArgs* e)
	{
	...
	void CControlEditor2::KM_MouseEnter(IControl* sender, EventArgs* e)
	{
	...
	void CControlEditor2::KM_MouseLeave(IControl* sender, EventArgs* e)
	{
	...
	void CControlEditor2::KM_Click(IControl* sender, EventArgs* e)
	{
	...
	void CControlEditor2::RebuildMesh()
	{
		if(_Locked)
			return;
...
// 		ItControls.clear();
		...
		for(unsigned int c = 0; c < Categories.size(); ++c)
		{
			...
			for(unsigned int i = 0; i < Categories[c].Instances.size(); ++i)
			{
				...
				KM->MouseEnter()->AddEvent(this, &CControlEditor2::KM_MouseEnter);
				KM->MouseLeave()->AddEvent(this, &CControlEditor2::KM_MouseLeave);
				KM->Click()->AddEvent(this, &CControlEditor2::KM_Click);
	...
	void CControlEditor2::SetLayout(IControlLayout* layout)
	{
	...
	void CControlEditor2::OnSizeChange()
	{
		IControl::OnSizeChange();
		this->RebuildMesh();
	}

	ControlEventHandler* CControlEditor2::WaitForInputKeyCode() const
	{
