...
Version: 1.4.1
Released: 7th of September 2008

Copyright (C) 2007-2008 Aaron Torpy (aaron@bundysoft.com)
...
extern "C" __declspec(dllexport) void* LoadHFZ(const char* path)
{
	hfzHeader Header;
	memset(&Header, 0, sizeof(Header));
	float* Data;

	if(hfzLoadEx(path, Header, &Data, NULL, NULL) != LIBHFZ_STATUS_OK)
		return 0;
	
	int Length = Header.nx * Header.ny;

	float* oData = (float*)malloc(sizeof(float) * (Length + 1));
	memcpy(oData + 4, Data, sizeof(float) * Length);
	free(Data);
	((int*)oData)[0] = Length;
...
extern "C" __declspec(dllexport) void FreePointer(void* Ptr)
{
	free(Ptr);
}
...
long hfzRead(hfzFile* fs, void* pData, long len) {
	...
	if(hfzFreadProc) {
		...
		switch(fs->Format) {
		case LIBHFZ_FORMAT_HF2:    return (long)fread(pData, len, 1, (FILE*)fs->pIoStream);
