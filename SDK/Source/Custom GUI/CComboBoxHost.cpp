...
#include "CComboBoxHost.h"

namespace RealmCrafter
{
	namespace SDK
	{
		CComboBoxHost::CComboBoxHost(IControlHostManager* manager, IControlHost* master)
			: IControlHost(manager, master), Handle(NULL), SelectedIndex(0)
		{

		}

		CComboBoxHost::~CComboBoxHost()
		{
		...
		NGin::GUI::IControl* CComboBoxHost::GetHandle()
		{
			return Handle;
		}

		bool CComboBoxHost::ReadProperty(PacketReader* reader, std::string &name)
		{
			// Default properties
		...
		void CComboBoxHost::Create(IControlHostManager* manager, ControlFactoryEventArgs* e)
		{
			// Setup ControlHost
			CComboBoxHost* Host = new CComboBoxHost(manager, e->Master);
			if(e->Master == NULL)
				Host->Master = Host;
			...
			// Create ComboBox handle
			Host->Handle = manager->GetGUIManager()->CreateComboBox("UserComboBox", NGin::Math::Vector2(0, 0), NGin::Math::Vector2(75.0f, 23.0f) / manager->GetGUIManager()->GetResolution());
			Host->Handle->SelectedIndexChanged()->AddEvent(Host, &CComboBoxHost::Handle_SelectedIndexChanged);
		...
		void CComboBoxHost::Handle_SelectedIndexChanged(NGin::GUI::IControl* sender, NGin::GUI::EventArgs* e)
		{
			PacketWriter Pa;
			Pa.Write(AllocID);
